...
router.post('/', [
	...
	User.findOne({ email }, function (err, user) {
		...
		User.findOneAndUpdate({ email }, { recoveryCode }, { new: true })
			.then(function (us) {

				sendAnEmail(email, recoveryCode, us.name).catch(console.error);
	...
	User.findOne({ email }, function (err, user) {
		...
		if (recoveryCode == user.recoveryCode) {
			bcrypt.hash(password, 10, function (err, hash) {
				User.findOneAndUpdate({ email }, { recoveryCode: '', password: hash }, { new: true })
					.then(function (us) {
