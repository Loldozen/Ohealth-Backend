...
router.post('/', [
	...
	let { currentToken, doctorID, appointmentID, endTime2 } = request.body
	let duration = ''
	Doctor.findOne({ _id: doctorID, currentToken }, function (err, doctor) {
		...
		Appointment.findOne({ _id: appointmentID, doctorID }, async function (err, appointment) {
			if (!appointment) {
				//Invalid request
				return response.status(400).json({ error: "Something went wrong, please contact the customer care" })
			}
			...
			if ((appointment.type == "Audio") && (appointment.status == "Ongoing")) {
				...
				let resp = await setWalletUser(diff, appointment.userID)
				let tran = { appointmentID, userID: appointment.userID, doctorID, type: 'Credit', appointmentType: 'Audio', status: 'Completed' }
				let resp2 = await setWalletDoctor(diff, doctorID, tran)
				...
				let resp = await setWalletUser(diff, appointment.userID)
				let tran = { appointmentID, userID: appointment.userID, doctorID, type: 'Credit', appointmentType: 'Video', status: 'Completed' }
				let resp2 = await setWalletDoctor(diff, doctorID, tran)
			}



			Appointment.findOneAndUpdate({ _id: appointmentID }, { status: 'Completed' }, { new: true })
...
const setWalletUser = async (diff, userID) => {
	try {
		let user = await User.findOne({ _id: userID })
		let re = await User.findOneAndUpdate({ _id: user._id }, { wallet: (user.wallet - diff).toFixed(2) }, { new: true })
	}
	catch (error) {
		console.log(error)
	}
